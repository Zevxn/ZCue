...
- `WH_KEYBOARD_LL` 收到按键时，目标控件和输入法可能还没有处理这次按键。普通字符路径不能立即用物理按键缓冲调用 `RecomputeSuggestions`：输入法预编辑中的拼音会暂时像普通文本一样匹配，并在输入法候选窗出现前抢先显示 ZCue 候选。应先更新缓冲、隐藏过期候选，再通过 `ScheduleFocusedTextSync` 延迟检查组合状态后决定是否重算。
- 组合态判断同时使用 `ImeCompositionService.IsComposing` 和可见输入法候选窗。进入选词阶段后，IMM32 组合串可能已经为空，因此不能只看 `IsComposing`。候选窗检测目前枚举顶层窗口，可能受其他应用或常驻候选窗影响；若要缩小检测范围，必须保留选词阶段的覆盖能力，并实际验证目标输入法，不能直接删除候选窗信号。
- 输入法提交中文后，物理缓冲里可能仍是拼音，不能因光标前文本不以拼音 token 结尾就清空缓冲。若刚观察到 IME 组合且 `FocusedTextService` 能读到光标前文本，应以该真实文本同步输入缓冲，再执行中文名称匹配。仅在 Shift 切换英文并提交时保留物理输入；UI Automation 读不到文本的 Chromium 控件继续使用拼音匹配结果推断已提交的中文片段。
- 回归来源：`76e9b03` 保留了延迟重算、候选窗检测和 IME 提交后的真实文本同步；`a79aa89` 加入普通字符即时重算、移除候选窗检测，并在物理拼音与已上屏中文不一致时清空缓冲，分别导致预编辑候选闪现和中文名称匹配丢失。修改这条链路前应对照这两个提交检查完整输入时序。
- 手动验证至少覆盖：拼音预编辑和候选选择期间 ZCue 候选不显示；中文上屏后中文名称仍可匹配；普通拉丁输入、Backspace、Esc、空格/回车及 VS Code 等无法可靠读取光标文本的控件行为正常。
